...
numsamp=10
for loop_num in range(3,4): #range(1, 6):
    for ver in version:
        for sub in subs:
            for rec in recs:
                for numsamp in number_samps:
                    print(f"Processing SHAP Classical act: {acts}, sub: {sub}, rec: {rec}")
                    # Read the data                   
                    #  df_train = pd.read_csv(f'/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/X_train_{rec}rec_sub{sub}act{act}v{version}.csv')
                    #  df_test = pd.read_csv(f'/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/X_test_{rec}rec_sub{sub}act{act}v{version}.csv')
                    #  df_ytrn = pd.read_csv(f'/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/y_train_{rec}rec_sub{sub}act{act}v{version}.csv', usecols=['Act'])
                    #  df_ytst = pd.read_csv(f'/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/y_test_{rec}rec_sub{sub}act{act}v{version}.csv', usecols=['Act'])
        
                    # # # Run algorithms and store results
                    # columns_to_keep=df_train.columns
                    # results_dict_qrc = runalgos1(df_train, df_test, df_ytrn, df_ytst, columns_to_keep)
                    ...
                    # version = 2
                    #Import Data Embeddings back from Julia QRC                                                merck_test_embeddings_recs100_sub5act14v3noise_qubits18_milan_sampling_v3 
                    train_emb = pd.read_csv(f"./records{rec}/merck_train_embeddings_recs{rec}_sub{sub}act{acts}v{ver}noise_qubits18_milan_sampling_v{ver}_numsamp{numsamp}_loop{loop_num}.csv").T
                    test_emb = pd.read_csv(f"./records{rec}/merck_test_embeddings_recs{rec}_sub{sub}act{acts}v{ver}noise_qubits18_milan_sampling_v{ver}_numsamp{numsamp}_loop{loop_num}.csv").T
                    y_train_emb = pd.read_csv(f"./records{rec}/merck_train_outcomes_recs{rec}_sub{sub}act{acts}v{ver}noisequbits18_milan_sampling_v{ver}_numsamp{numsamp}_loop{loop_num}.csv", header=None)
                    y_test_emb = pd.read_csv(f"./records{rec}/merck_test_outcomes_recs{rec}_sub{sub}act{acts}v{ver}noisequbits18_milan_sampling_v{ver}_numsamp{numsamp}_loop{loop_num}.csv", header=None)
                    print(f"Processing Embedding act: {acts}, sub: {sub}, rec: {rec}")
                    ...
                    results_df_qrcemb['act'     ] = acts
                    results_df_qrcemb['sub'] = sub
                    results_df_qrcemb['rec'] = rec
                    results_df_qrcemb['numsamp'] = numsamp
                    results_df_qrcemb['ver'] = ver
                    results_df_qrcemb['source'] = 'results_dict_qrcemb'
                    all_results_df = pd.concat([all_results_df, results_df_qrcemb], ignore_index=True)
                    #version = 3
                    print(f"Processing Linear Embedding act: {acts}, sub: {sub}, rec: {rec}, ver: {ver}")
        
                    # # Import Data Embeddings back from Julia QRC (Linear)
                    # train_emb_lin = pd.read_csv(f"/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/records{rec}/merck_train_emb_lin_rec{rec}rec_sub{sub}act{act}v{version}.csv", header=None).T
                    # test_emb_lin = pd.read_csv(f"/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/records{rec}/merck_test_emb_lin_rec{rec}rec_sub{sub}act{act}v{version}.csv", header=None).T
                    # y_train_emb_lin = pd.read_csv(f"/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/records{rec}/merck_train_outcomes_lin_rec{rec}rec_sub{sub}act{act}v{version}.csv", header=None)
                    # y_test_emb_lin = pd.read_csv(f"/home/dabeaulieu/quantum/beaulieu-dan/qrc/regression/records{rec}/merck_test_outcomes_lin_rec{rec}rec_sub{sub}act{act}v{version}.csv", header=None)
                    ...
                    # results_dict_qrcemblin = runalgos1(X_train_shapemblin, X_test_shapemblin, y_train_shapemblin, y_test_shapemblin, columns_to_keep)
    ...
    output_filename = f"./wavefunction_sampling_results_act{acts}_loop{loop_num}_rec{rec}_{current_date}_v{ver}.csv"
    all_results_df.to_csv(output_filename, index=False)
    print(f"Results saved to {output_filename}")
# Output the master DataFrame to a CSV file
#all_results_df.to_csv("./all_results_shapgrad_regular_sub5_act14_20250331_sampling_wavefunction.csv", index=False)
